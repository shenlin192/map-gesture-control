...
export class EMASmoother {
	...
	smooth(value: number): number {
		...
		const smoothedValue = this.alpha * value + (1 - this.alpha) * this.lastSValue;
		this.lastSValue = smoothedValue;
		return smoothedValue;
	}

	smoothLandmark(landmark: NormalizedLandmark | null): NormalizedLandmark | null {
		...
		const newSmoothed: NormalizedLandmark = {
			...
			z: landmark.z !== undefined && this.lastLValue.z !== undefined ?
				this.alpha * landmark.z + (1 - this.alpha) * this.lastLValue.z : landmark.z,
