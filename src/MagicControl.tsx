...
const MEDIAPIPE_GESTURE_MODEL_PATH: string = 'https://storage.googleapis.com/mediapipe-models/gesture_recognizer/gesture_recognizer/float16/1/gesture_recognizer.task';

// Types for better type safety
type GestureType = 'NONE' | 'PINCHING' | 'PANNING_POINTING_UP' | 'PANNING_CUSTOM';
...
function MagicControl() {
	...
	const velocityRef = useRef<{ x: number; y: number; lastTime: number } | null>(null);
	const flickMomentumRef = useRef<{ vx: number; vy: number; startTime: number } | null>(null);
	...
	// State
	const [gestureStatus, setGestureStatus] = useState<GestureStatus>('Initializing Map...');
	...
	const calculateDistance = useCallback((p1?: NormalizedLandmark, p2?: NormalizedLandmark): number => {
		if (!p1 || !p2) return Infinity;
		return Math.sqrt(Math.pow(p1.x - p2.x, 2) + Math.pow(p1.y - p2.y, 2) + Math.pow(p1.z! - p2.z!, 2));
	}, []);

	const getScreenCoordinatesForMap = useCallback((normalizedLandmark: NormalizedLandmark): [number, number] | null => {
	...
	const isIndexPointingCustom = useCallback((landmarks: NormalizedLandmark[]): boolean => {
		if (!landmarks || landmarks.length === 0) return false;
		const hand = landmarks;
		const idxT = hand[8], idxP = hand[6], idxM = hand[5];
		const midT = hand[12], midP = hand[10];
		const rngT = hand[16], rngP = hand[14];
		const pkyT = hand[20], pkyP = hand[18];
		if (!idxT || !idxP || !idxM || !midT || !midP || !rngT || !rngP || !pkyT || !pkyP) return false;
	...
	const processGestures = useCallback((landmarksList: NormalizedLandmark[][] | null, recognizedGestures?: Category[][]): void => {
		...
		try {
			...
			if (hand.length < 21) {
				console.debug('Incomplete hand data, expected 21 landmarks, got', hand.length);
				return;
			}
			...
			if (!thumbTip || !indexTip ||
				thumbTip.x === undefined || thumbTip.y === undefined ||
				indexTip.x === undefined || indexTip.y === undefined) {
				console.debug('Missing or invalid thumb/index tip data for first hand');
				return;
			}
			...
			if (landmarksList.length >= 2) {
				...
				if (secondHand && secondHand.length >= 21) {
					...
					if (!secondThumbTip || !secondIndexTip ||
						secondThumbTip.x === undefined || secondThumbTip.y === undefined ||
						secondIndexTip.x === undefined || secondIndexTip.y === undefined) {
						console.debug('Missing or invalid thumb/index tip data for second hand');
			...
			const currentRecGesture = recognizedGestures?.[0]?.[0]?.categoryName || 'NONE';
			...
			const twoHandsDetected = landmarksList.length >= 2 && secondHand !== null;
			console.log(`Hands detected: ${landmarksList.length}, Two hands: ${twoHandsDetected}`);
			...
			if (twoHandsDetected) {
				try {
					...
					console.log('Two-hand zoom - distance1:', dist1, 'distance2:', dist2, 'avg:', avgPinchDistance);

					// Process two-hand zoom
					if (handlePinchToZoom(hand, thumbTip, indexTip, currentRecGesture, midPoint, avgPinchDistance)) {
						return;
					}
				} catch (error) {
					console.error('Error in two-hand zoom:', error);
					// Fall back to single-hand zoom if there's an error
					if (handlePinchToZoom(hand, thumbTip, indexTip, currentRecGesture, undefined, currentDist)) {
						return;
					}
				}
			} else {
			...
			if (controlMode === 'ZOOM') {
				...
				if (twoHandsDetected && landmarksList.length >= 2) {
					try {
						...
						const secondHandMid: NormalizedLandmark = {
							...
							visibility: (secondThumbTip.visibility + secondIndexTip.visibility) / 2
						};

						// Calculate distance between hands for two-hand zoom
						const handsDistance = calculateDistance(firstHandMid, secondHandMid);
						const secondHandDist = calculateDistance(secondThumbTip, secondIndexTip);
						const avgPinchDistance = (currentDist + secondHandDist) / 2;

						console.log(`Two-hand zoom - Hand1 dist: ${currentDist.toFixed(3)}, Hand2 dist: ${secondHandDist.toFixed(3)}, Hands distance: ${handsDistance.toFixed(3)}`);

						// Use the average of both hands for zooming
						handlePinchToZoom(hand, thumbTip, indexTip, currentRecGesture, {
					...
					if (handlePinchToZoom(hand, thumbTip, indexTip, currentRecGesture, undefined, currentDist)) {
						...
						if (currentDist > 0.25) {  // Only switch if fingers are not pinching
							modeTimeoutRef.current = setTimeout(() => {
				...
				try {
					if (handlePanning(hand, indexTip, currentRecGesture, customPointing)) {
						return;
					}
				} catch (error) {
			...
			setGestureStatus(`Mode: ${controlMode} | Gesture: ${currentRecGesture}`);
		} catch (error) {
	...
	const handlePinchToZoom = (
		...
		try {
			...
			const currentDist = customDistance ?? calculateDistance(thumbTip, indexTip);
			...
			const currentMidpointScreen = getScreenCoordinatesForMap(midpoint as NormalizedLandmark);
			if (!currentMidpointScreen) {
				console.log('Could not get screen coordinates for midpoint');
				return false;
			}

			console.log(`Zoom - Distance: ${smoothedDist.toFixed(3)}, Midpoint: (${midpoint.x.toFixed(3)}, ${midpoint.y.toFixed(3)})`);
			...
			if (activeGestureRef.current !== 'PINCHING') {
				...
				console.log('Pinch started - initial distance:', smoothedDist.toFixed(3));
				return true;
			}
			...
			const zoomDelta = (initialPinchDistanceRef.current - smoothedDist) * ZOOM_SENSITIVITY;
			let newZoom = currentZoom + zoomDelta;

			console.log('Zoom delta:', zoomDelta.toFixed(3), 'New zoom:', newZoom.toFixed(3));
			...
			// Debug logging
			console.log(
				'Pinch Dist:', smoothedDist.toFixed(4),
				'Zoom Delta:', zoomDelta.toFixed(4),
				'Current Zoom:', currentZoom.toFixed(2),
				'New Zoom:', newZoom.toFixed(2));
			...
			setGestureStatus(`Zoom: ${newZoom.toFixed(2)} (${zoomDelta > 0 ? '+' : ''}${zoomDelta.toFixed(3)})`);
	...
	const applyMomentum = useCallback(() => {
		...
		const mapInstance = mapRef.current?.getMap();
		if (mapInstance) {
			mapInstance.panBy([-currentVx, -currentVy], {duration: 50, animate: true});
		}
	...
	const handlePanning = (
		...
		try {
			console.log('handlePanning called with:', {hand, indexTip, currentRecGesture, customPointing});
			...
			console.log('Pan check - distance:', currentDist, 'threshold:', PAN_DISTANCE_THRESHOLD);

			// Only check for pinch interference if we're not already in a pan gesture
			if (activeGestureRef.current !== 'PANNING_POINTING_UP' &&
			...
			const statusText = isPointingUp ? 'Panning (Pointing Up)' : 'Panning (Custom Point)';
			...
			if (timeSinceLastFlick < 300) { // 300ms cooldown after flick
				lastPanPointRef.current = currentPanScreenPoint;
				return true;
			}
			...
			try {
				...
				mapInstance.panBy([-scaledDx, -scaledDy], {duration: 50, animate: true});
				lastPanPointRef.current = currentPanScreenPoint;
			} catch (panError) {
	...
	useEffect(() => {
		if (gestureRecognizerRef.current || gestureStatus !== 'Map Loaded. Initializing MediaPipe...') {
			return;
		}
		...
		const initializeMediaPipe = async () => {
			try {
				...
				landmarkSmootherRef.current = Array(21).fill(null).map(() => new EMASmoother(0.4));
				setGestureStatus('MediaPipe Ready. Starting Webcam...');
			} catch (err) {
				const errorMessage = 'Error initializing MediaPipe: ' + (err instanceof Error ? err.message : 'Unknown error');
	...
	// --- Initialize Webcam ---
	useEffect(() => {
		if (!gestureRecLoaded || gestureStatus !== 'MediaPipe Ready. Starting Webcam...') {
			return;
		}
		...
		const setupWebcam = async () => {
			try {
				...
				const errorMessage = 'Error accessing webcam: ' + (err instanceof Error ? err.message : 'Unknown error');
				console.error(errorMessage, err);
		...
		<div className="w-full flex flex-col h-screen bg-gray-800 text-white items-center p-4 font-sans">
			...
			<div className="w-full flex flex-col md:flex-row gap-4">
				...
				<div className="w-full md:w-1/3 flex flex-col items-center gap-3">
					...
					<StatusDisplay gestureStatus={gestureStatus} activeGesture={activeGesture}/>
				</div>
			</div>
		</div>
	);
